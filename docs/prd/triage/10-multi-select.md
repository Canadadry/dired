...
Let the user select multiple entries and apply an action (delete/trash/copy/move) to all of them at once, using the action queue from `09-async-action-queue` to run the resulting batch without freezing the UI.
...
4. As a developer, I want a batch of actions from a multi-select operation to run through the async action queue (`09`), so that a large batch doesn't block the UI.

## Implementation Decisions

**Decided:**
- Multi-select is conceptually the consumer of `09-async-action-queue`'s queue for bulk operations — deliberately sequenced last in the roadmap, after the queue mechanism exists.
...
- The queue mechanism itself (`09-async-action-queue`).
- The specific batch operations' individual semantics (`03-copy-move`, `07-trash`).

## Further Notes

Depends on `01-foundation` and `09-async-action-queue`. Sequenced last in the roadmap.
