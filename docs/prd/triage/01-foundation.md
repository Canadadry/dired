...
> **Draft status:** written from the high-level architecture/roadmap grilling session (covers the whole 10-item roadmap), not from a dedicated grilling session on this PRD specifically. Treat the decisions below as the current best understanding, not final — this PRD needs its own focused grilling pass before it leaves triage.
...
- **main() loop**: `tb_init()` → construct the initial `Model` (plus an initial `Cmd` to load the current directory, mirroring the Elm `init` pattern) → loop: run pending `Cmd` if any → `view(model)` → paint via termbox2 → `tb_poll_event()` → translate event to `Msg` → `update(msg, model)` → repeat. `main()` is the only function calling any `tb_*` function or any filesystem/process function (`rename`, `mkdir`, `fopen`, `opendir`, `fork`/`exec`).
...
## Open Questions (needs dedicated grilling)

- The "`main()` builds an initial `Model` + runs an initial `Cmd` to load the current directory" startup sequence was inferred by analogy to Elm's `init`, not explicitly confirmed with the user.
- Exact `Msg` and `Cmd` variant names/vocabulary, and the exact set of semantic style tags `view()` emits, are not pinned down — only the shape of the mechanism is.
- Whether the current fixed-size limits (`MAX_ENTRIES 1024`, `NAME_MAX_LEN`, `PATH_MAX_LEN`) carry over unchanged into `Model`, or should become dynamic.
- How the current "virtual line" concept (the inline editable row shown while creating a file/directory) maps onto the new explicit `Model` states — not re-discussed after the state-machine redesign.
- Whether `view()`'s single-list-of-lines output should already be shaped as "one panel" (a list containing one panel) to minimize the rework the file-preview PRD will need, or stay a flat list as originally described.
