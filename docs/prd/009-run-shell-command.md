...
- **New `Cmd`**: `CMD_RUN`, carrying the existing `path` field (reused to mean "working directory to run in", set to `current_path`), `cmd_text[PATH_MAX_LEN]` (the command string with its leading `!` already stripped off), and `selected_path[PATH_MAX_LEN]` (the currently selected entry's full path, or empty if nothing is selected/the directory is empty).
- **`$FILE` env var**: `execute_run_cmd` exports the selected entry's path as `$FILE` (`setenv`) before running the command, `unsetenv`-ing it when nothing is selected so a stale value from a previous run can't leak through. This lets a command reference the selection directly, e.g. `!unzip $FILE` or `!cp $FILE /backup/`, without retyping the filename. `update()` computes `selected_path` once, in the same `handle_edit`/`MSG_ACTIVATE` branch that builds the rest of `CMD_RUN`, by joining `current_path` with `entries[selected].name` — pure and covered by the same table-driven test as the `!`-prefix parsing.
- **Prompt/label**: `add_prompt_line` in `view.c` gains a `MODE_RUN_CMD` case showing `":%s"` with the live `edit_buf` appended directly on the prompt line itself (e.g. typing `!ls -la` renders `:!ls -la` on line 2, right under `Path:`) — a vim-style `:` command line, not a virtual row appended after the file listing. `model_has_virtual_line` is **not** extended for `MODE_RUN_CMD` (it stays `MODE_CREATE`-only): a bottom-of-listing virtual row (`MODE_CREATE`'s live-echo behavior) put the typed text below a potentially long, paginated file list, out of sight of where `:` was pressed — confirmed wrong in manual testing after the initial implementation.
...
- **Execution** (`execute_run_cmd(cwd, cmd_text, selected_path)` in `dired.c`): `tb_shutdown()`, sets/unsets `$FILE` per above, then the existing `run_piped` helper runs `{"/bin/sh", "-c", cmd_text, NULL}` as the writer piped into `{"more", NULL}` as the reader — the same helper already used for hexdump-into-pager. The writer child `chdir(cwd)`s immediately before `execvp`, which does not violate `001-foundation.md`'s "no `chdir()`" rule (that rule concerns the main process never relying on relative paths; a forked child that chdirs immediately before exec-ing and exiting never affects the main process's cwd).
